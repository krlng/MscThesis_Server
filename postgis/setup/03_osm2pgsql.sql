...
CREATE OR REPLACE VIEW planet_osm_line_filtered AS 
 SELECT planet_osm_line.osm_id,
  ...
  WHERE planet_osm_line.highway = ANY (ARRAY['motorway'::text, 'motorway_link'::text, 'primary'::text, 'primary_link'::text, 'secondary'::text, 'secondary_link'::text, 'tertiary'::text, 'tertiary_link'::text, 'track'::text]);

ALTER TABLE planet_osm_line_filtered
  OWNER TO nicokreiling;
